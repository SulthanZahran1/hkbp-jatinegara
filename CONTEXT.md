...
## Project
Modernizing HKBP Jatinegara's church administration from a legacy PHP stack (Laravel thin client + separate backend API) to **TypeScript (SPA frontend) + Go (API backend)**.
...
- **TypeScript SPA** — browser client, communicates with Go backend via JSON API
- **Go API** — single backend serving all business logic and data access
...
## Multi-Tenancy Note
The legacy architecture separates the frontend (Laravel) from the backend (a separate API at `dbruas-be.hkbpjtn.web.id`). The new architecture collapses this into a single Go API + TS SPA.
